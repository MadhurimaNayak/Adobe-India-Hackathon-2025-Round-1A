...
def union_make(pdf_info_dict: list,
    ...
    for page_info in pdf_info_dict:
        ...
        if make_mode in [MakeMode.MM_MD, MakeMode.NLP_MD]:
            ...
            return make_structured_content_list(pdf_info_dict)  
            '''
            for para_block in paras_of_layout:
...
def make_structured_content_list(pdf_info_dict: list):  
    """Generate structured content list with title and outline"""  
    title_blocks = []  
      
    # Collect all title blocks across pages  
    for page_info in pdf_info_dict:  
        paras_of_layout = page_info.get('para_blocks', [])  
        page_idx = page_info.get('page_idx', 0)  # Already 0-based  
          
        for para_block in paras_of_layout:  
            if para_block['type'] == BlockType.TITLE:  
                title_level = get_title_level(para_block)  
                if title_level > 0:  # Valid title level  
                    title_text = merge_para_with_text(para_block)  
                    title_blocks.append({  
                        'text': title_text,  
                        'level': title_level,  
                        'page': page_idx  # Keep 0-based indexing  
                    })  
      
    # Determine document title (first title or highest level title)  
    document_title = ""  
    if title_blocks:  
        # Find the highest level (lowest number) title  
        highest_level = min(block['level'] for block in title_blocks)  
        first_highest_title = next(block for block in title_blocks if block['level'] == highest_level)  
        document_title = first_highest_title['text']
        title_block_to_exclude = first_highest_title  
      
    # Build outline with H1, H2, H3 format  
    outline = []  
    for block in title_blocks:  
        if block is title_block_to_exclude:  
            continue 
        level_str = f"H{block['level']}"  
        outline.append({  
            'level': level_str,  
            'text': block['text'],  
            'page': block['page']  # 0-based page indexing  
        })  
