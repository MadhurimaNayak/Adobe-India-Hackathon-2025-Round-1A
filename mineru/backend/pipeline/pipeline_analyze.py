...
def doc_analyze(
    ...
    """
    min_batch_inference_size = int(os.environ.get('MINERU_MIN_BATCH_INFERENCE_SIZE', 348))
...
def batch_image_analyze_p(  
        images_with_extra_info: List[Tuple[PIL.Image.Image, bool, str]],  
        formula_enable=False,  
        table_enable=False,  
        num_workers=8):  
      
    from .multi_cpu_processor import MultiCPUProcessor  

      
    # Use multi-CPU processor for parallel processing  
...
def worker_init(lang, formula_enable, table_enable):  
    """Initialize models once per worker process"""  
    global pipeline_model  
    pipeline_model = custom_model_init(  
        lang=lang,  
        formula_enable=formula_enable,  
        table_enable=table_enable  
    )      
  
def worker_function(chunk_data):  
    """Worker function that processes a chunk of images"""  
    global pipeline_model  
    from .batch_analyze import BatchAnalyze  
      
    images_with_extra_info, formula_enable, table_enable = chunk_data  
      
    # Wrap the pipeline model to provide get_model interface  
    model_manager = ModelManagerWrapper(pipeline_model)  
    batch_model = BatchAnalyze(model_manager, 1, formula_enable, table_enable)  
    return batch_model(images_with_extra_info)  
  
def batch_image_analyze_parallel(images_with_extra_info, formula_enable=False, table_enable=False):  
    """Parallel version of batch_image_analyze"""  
    total_pages = len(images_with_extra_info)  
    num_processes = min(mp.cpu_count(), 4)  
      
    # Split into chunks  
    chunk_size = max(1, total_pages // num_processes)  
    chunks = [(images_with_extra_info[i:i + chunk_size], formula_enable, table_enable)  
              for i in range(0, total_pages, chunk_size)]  
      
    with Pool(processes=num_processes,   
              initializer=worker_init,   
              initargs=('en', formula_enable, table_enable)) as pool:  
        chunk_results = pool.map(worker_function, chunks)  
